...
exports.index = async (req, res, next) => {
  if (require('../API_KEY/data/check_api_key.js').check_api_key(req, res)) return;
	...
	const pathBg = __dirname + `/cache/bbgg.png`;
	const pathAva = __dirname + `/cache/av.png`;
	const pathLine = __dirname + `/cache/li.png`;
	...
	const avt = (await axios.get(encodeURI(`https://graph.facebook.com/${uid}/picture?height=1500&width=1500&access_token=6628568379%7Cc1e620fa708a1d5696fb991c1bde5662`), {
		responseType: "arraybuffer"
	})).data;
	const background = (await axios.get(encodeURI(`https://i.imgur.com/OC7ZYE6.png`), {
		responseType: "arraybuffer"
	})).data;
	const hieuung = (await axios.get(encodeURI(`https://i.imgur.com/ETTWIEL.png`), {
		responseType: "arraybuffer"
	})).data;
	...
	if (!fs.existsSync(__dirname + `/cache/UTMAvoBold.ttf`)) {
		const getfont2 = (await axios.get(`https://drive.google.com/u/0/uc?id=1DuI-ou9OGEkII7n8odx-A7NIcYz0Xk9o&export=download`, {
			responseType: "arraybuffer"
		})).data;
	...
	if (!fs.existsSync(__dirname + `/cache/Baloo Regular.ttf`)) {
		const getfont1 = (await axios.get(`https://drive.google.com/u/0/uc?id=1IrxrZxo1ht3jur4ZI5MxH9Ri6HspO6YS&export=download`, {
			responseType: "arraybuffer"
		})).data;
	...
	ctx.textAlign = "center";
	const l = ctx.measureText(name).width;
